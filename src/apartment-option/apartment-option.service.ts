...
export class ApartmentOptionService extends BaseService<ApartmentOptionEntity> {
  ...
  async create(
    createApartmentOptionDto: CreateApartmentOptionDto,
  ): Promise<ApartmentOptionEntity> {
  ...
  async update(
    id: number,
    updateApartmentOptionDto: UpdateApartmentOptionDto,
  ): Promise<ApartmentOptionEntity> {
    const option: ApartmentOptionEntity = await this.repository.findOne({
      where: { id },
    });
    if (!option) {
